      ...
      complex*16 fomega,cx,comega,comeg1,comeg2,dcomega,cerr,cerra,
     $      cdh,dror,
...
c
	 call eigenf(nmax,omsq,n,lval,rke,gor)
c
...
c      if( cdabs(comega) .gt. 1.1d0*omega ) comega = omega*(1.d0,1.d-4)
      do 55 icount=1,30
         ...
         elseif( icount .eq. 2 ) then
            ...
            if( abs(dcomega)/abs(comega) .gt. 1.d-4 ) then
               dcomega = 1.d-4*abs(comega)*dcomega/abs(dcomega)
            endif
c            comega = comeg2
            ...
            cerra = cerr
            dcomega = comeg2 - comega
            if( abs(dcomega)/abs(comega) .gt. 0.01 ) then
               dcomega = 0.01d0*abs(comega)*dcomega/abs(dcomega)
            endif
c            comega = comeg2
            comega = comega + dcomega
         endif
...
c      call cptdmp(dror,nmax,np1,'dr/r')
c      call cptdmp(cdh,nmax,np1,'dh/h')
c      call cptdmp(drho,nmax,np1,'drho')
c      call cptdmp(dent,nmax,np1,'dscv')
c      call cptdmp(dlum,nmax,np1,'dl/l')
c
