...
c
c       05/31/01: Conversion to Fortran 90 begun
c
      parameter ( nmax=8192 )
...
c      open (unit=11,file='polout.rk',status='unknown')
c      open (unit=12,file='polplt.rk',status='unknown')
c      open (unit=33,file='polfreqs.txt',status='unknown',
c     $      position='append')
c
...
c
c      close(12)
      write(plot_filename, 1010) int( 1000.*fnpol ), npts
      open (unit=12,file=plot_filename,status='unknown')
      write(plot_filename, 1015) int( 1000.*fnpol ), npts
      open (unit=11,file=plot_filename,status='unknown')
      write(plot_filename, 1016) int( 1000.*fnpol ), npts
      open (unit=33,file=plot_filename,status='unknown',
     $      position='append')
      write(33,*) fnpol
 1010 format('polyrk_',i4.4,'_'i5.5,'.plt')
 1015 format('polyrk_',i4.4,'_'i5.5,'.out')
 1016 format('polyrk_',i4.4,'_'i5.5,'.txt')
c
      idonpl = 1
...
c
      ...
      do 10 i=1,npuls
         ...
         g1(i) = 5.d0/3.d0
         c2(i) = p(i)*v(i)*g1(i)
         asym_p = asym_p + (r(i+1)-r(i))/dsqrt(c2(i))
         c2(i) = dlog10(c2(i))
  10  continue
      asym_p = one/asym_p
c
...
c
      ...
      if( dabs(bvfac) .gt. 1.d-12 ) then
         do i=1,npuls
            rai = (bvfac/alfa)*dthdr(i+1)/(theta(i)*xi(i+1))
            ra(i) = rai
            bv(i) = (rai)*gor(i)/om02
            rz_i = (r(i+1)+r(i))/2.d0
            asym_g = asym_g + (r(i+1)-r(i))*dsqrt(abs(bv(i)))/rz_i
         enddo
         asym_g = dsign( one/asym_g, bvfac )
      endif
c
      igmax = 0
      do 16 i=1,npuls
...
c
c      call date( dat_buf )
c      call time( time_buf )
      call date_and_time( dat_buf, time_buf )
c
      write(6,2000) npuls, time_buf, dat_buf
      write(11,2000) npuls, time_buf, dat_buf
...
c
 100  continue
         write(6,1001)
         read(5,*,end=900) iopuls,lvalin,nomega_in,ihmax,omlow,omhigh
         if( lvalin .lt. 0 ) goto 910
         om02 = pi*g*rhobar
         omlow = omlow*om02
         omhigh = omhigh*om02
         nmode = 0
	 do ldum= 0,lvalin
            lval = ldum
            nomega = nomega_in
	    if( lval .eq. 0 ) nomega=1
	    write(6,*) ldum, nomega, ihmax
...
c
 ...
 1001 format(1x,'Pulsation parameters,',/,
     $   1x,'io, l, nom, ihmax, omlow, and omhigh: ',$)
 1700 format(1x,15hmax. g in zone ,i4,9h radius =,1pe10.3,
     $      8H mass = ,e10.3)
 1099 format(1x,'Asymptotic frequency for p modes: ',1pe11.4,
     $          ', g modes: ',e11.4)
 2000 format(41h1 polytropic model in hydrostatic balance,//,
     $      1x,i5,' zones, built at ',a8,' on ',a9,/)
 ...
 2007 format(1h1,/,1x,18houtput from polyrk,i5,' zones, built at ',
     $   a8,' on ',a9,/,
...
c
         ...
         dthdr(1) = zero
         xi(2) = dr
c         theta(2) = one - dr**2*(one-fnpol*dr**2/20.d0)/6.d0
c         dthdr(2) = dr**3*(one-fnpol*dr**2/10.d0)/3.d0
c
c          Expansion from Hansen and Kawaler, eq. 7.45
c
         theta(2) = one - dr**2*(one-dr**2*(fnpol/20.d0)*
     $             (one-dr**2*(8.d0*fnpol-5.d0)/126.d0) )/6.d0
         dthdr(2) = dr**3*(one-dr**2*(fnpol/10.d0 -
     $              dr**2*fnpol*(8.d0*fnpol-5.d0)/840.d0) )/3.d0
         x(1) = dr
         y(1) = theta(2)
         y(2) =-dthdr(2)/dr**2
         do 30 ic=2,n+1
  ...
  35        continue
  31       continue
c           write(6,*) ic, fnpol, x(1), y(1), y(2)
            np = ic + 1
            xi(np) = x(1)
            theta(np) = y(1)
...
c
 ...
 5500 format(/,5x,5hfnpol,f7.3,8H radius=,1pe16.9)
...
c
     ...
     $      yo(nmax),stwait(nmax),epstwt(nmax),spac(nmax,5)
      dimension drz(nmax),rkl2(nmax),drint(nmax), drho_Eul(nmax)
...
c
      ...
      do 10 i=1,n
         ...
         drint(i) = r(i+1)-r(i)
c         drdm(i,1) = forpi*rzone(i)/(dm1(i)*v(i)) -
c     $    ahf*(rl+one)/rzone(i)**2
c         drdm(i,2) =-(forpi*rzone(i)/(dm1(i)*v(i)) +
c     $    ahf*(rl+one)/rzone(i)**2)
         drdm(i,1) = (rzone(i)/drint(i) - ahf*(rl+one))/rzone(i)**2
         drdm(i,2) =-(rzone(i)/drint(i) + ahf*(rl+one))/rzone(i)**2
  10  continue
      rkl2(1:n) = rl1/rzone(1:n)**2
      z(1:n) = rzone(1:n)/r(np1)
c
...
c
c      afac = forpi*r(1)**3/dm2(1)
c      drdm01 =-ahf*(rl+one)/rz0**2
c      drdm02 =-ahf*(rl+one)/rz0**2
c
c          Second form
c
c      drdm01 =-(23.d0/24.d0)*(rl+one) !/rz0**2
c      drdm02 = (5.d0/12.d0)*(rl+one)  !/rz0**2
c
c          Third form
c
      afac = zero
      rkl20 = rl1*(one/thre/rz0)**2
      drdm01 =-(7.d0/12.d0)*(rl+one)*(one/thre/rz0)**2
      drdm02 =-(5.d0/12.d0)*(rl+one)*(one/thre/rz0)**2
c
      p0 = P(1)
      rho0 = one/v(1)
c
     ...
     $     drdm01*g1(1)*p0*(ahf*rl/rho0 - afac )
      ag1(1,3) = drdm(1,2)*g1(1)*p(1)*(ahf*rl*v(1)+afac)
c      ag1(1,3) = ag1(1,3) + ag11
      ag1(1,3) = ag1(1,3) + drdm02*g1(1)*p(1)*(ahf*rl*v(1)-afac)
      ag3(1,1) = zero
      ag3(1,2) = rl
      ag41 = rkl20*(g1(1)*p0*(ahf*rl/rho0-afac))
      ag4(1,1) = zero
...
c          09/27/2001
c
c      afac = forpi*r(np1)**3/dm2(np1)
      afac = r(np1)*v(n)/drint(n)
      vl = g1(n)*p(n)*(rl*v(n) - afac)/
     $     (one+ahf*rl*drint(n)/r(np1))
      vl = -g1(n)*gor(np1)*r(np1)
c
c          10/14/2001: \delta P = 0 at outer interface
c
      vl = -two*r(np1)*g1(n)*p(n)*v(n)/drint(n)
      ag1(np1,1) = drdm(n,1)*vl
      fdr = (one + ahf*rl*(drz(np1)/r(np1)))/(one + dlnr)
...
c10/7/01            drho_Eul = adrho(i)+fnpol/(one+fnpol)*
c10/7/01     $               (rho(i)/P(i))*ahf*(gor(i)*dr(i) + gor(i+1)*dr(i+1))
            if( i .eq. 1 ) then
               drho_Eul(i) = adrho(i) + 
     $           rho(i)*gor(2)/p(1)*fnpol/(one+fnpol)
            else if( i .eq. n ) then
               drho_Eul(i) = adrho(i) + 
     $           rho(n)*gor(n+1)/p(n)*fnpol/(one+fnpol)
            else
               drho_Eul(i) = adrho(i) - ahf*(
     $        dlog(rho(i+1)/rho(i))/dlog(rzone(i+1)/rzone(i))*dr(i+1)+
     $        dlog(rho(i)/rho(i-1))/dlog(rzone(i)/rzone(i-1))*dr(i) )
            endif
            wgrav(i) = drho_Eul(i)*gam(i)*dm1(i) 
            if( i .eq. n ) then
               wcross(i+1) = gor(i+1)*r(i+1)*dr(i+1)*dm2(i+1)*
     $            2.d0*rl1*dh(i)
               wdiag(i+1) = ( pi4g*ahf*rho(i)   !(rho(i)+rho(i+1))
...
c
         ...
         write(33,3300) lval,index,np,ng, qomsq
...
c
     ...
     $                      weight(i),stwait(i),i=1,n)
         ...
         call pltdmp(drho_Eul, nmax, n, 'Erho')
...
c      do i=1, iroot
c         write(6,5600) (stomsq(i,j), j=1,4)
c      enddo
c 5600 format(1x,4(' &  ',f11.5))
c      call orthog(dr,dh,n,omsq,index,nmax,-1,lval)
      return
  50  continue
      ihmin = nomega
c
...
c
      ...
      write(33,3300) lval,iv,iv, 0, qomsq
c
c          normalize the weight function per zone to omsq.
c
      do 105 i=1,n
         strke(i+1) = strke(i+1)/rke
...
c
 ...
 1001 format(/,19h linear radial mode,/,
     ...
     $ 2x,'eigenvalue from weight function = ',e12.5,8H error =,e10.3,/,
...
c          n = 1 polytrope, l=2.
c
            omsq_comp = robcmp(index,omsq,omsq1)
         elseif( npol .eq. 20 ) then
c
c          n = 2 polytrope, l=2.
c
            omsq_comp = robcmp(index,omsq,omsq2)
         elseif( npol .eq. 30 ) then
c
c          n = 3 polytrope, l=2.
c
            omsq_comp = robcmp(index,omsq,omsq3)
         elseif( npol .eq. 35 ) then
c
c          n = 3.5 Polytrope, l=2.
c
            omsq_comp = robcmp(index,omsq,omsq35)
         elseif( npol .eq. 40 ) then
c
c          n = 4 polytrope, l=2.
c
            omsq_comp = robcmp(index,omsq,omsq4)
         else
            write(6,1000) fnpol
            omsq_comp = 0.d0
         endif
...
c
 ...
 1001 format(1x,4h k =,i3,8h omsq = ,1pe10.3,
     $   2X,12hreal omsq = ,e10.3,2X,7h error=,e12.3)
      ...
      function robcmp(index,omsq,omsqar)
      implicit real*8(a-h,o-z)
      dimension omsqar(21)
      if( omsq .le. 0. ) index = index-1
      if( iabs(index) .gt. 10 ) then
         robcmp = 0.d0
      else
         k = index + 11
         zomsq = omsqar(k)
