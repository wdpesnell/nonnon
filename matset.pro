function matset, nin, iout,lin, r,t,v,cv,dkdr, $
        dkdt,dm1,gkap,dm2,rm, bv, p,g1,g3m1,rho,rzone
...
;c      dkdr0 = dkdr0 + g3m10*dkdt0
      rho = one/v
;      rzone = (r[0:n-1]^3 + ahf*dm1[0:n-1]*v[0:n-1]/pi43)^onth
      rkl2 = rl1/rzone^2
      gor = g*rm/r^2
      drz = [ rzone[0]-rz0, rzone[1:n-1] - rzone[0:n-2], 0.]
      z = rzone/max(r)
      drint = r[1:n] - r[0:n-1]
      drdm = dblarr(n+1,2)
      drdm[0:n-1,0] = forpi*rzone[0:n-1]/(dm1[0:n-1]*v[0:n-1]) - ahf*(rl+one)/rzone[0:n-1]^2
      drdm[0:n-1,1] =-forpi*rzone[0:n-1]/(dm1[0:n-1]*v[0:n-1]) - ahf*(rl+one)/rzone[0:n-1]^2
      drdm[n,0] = 0.d0
      drdm[n,1] = 0.d0
      for i=1,n do begin
...
;c
         ...
         diff = t4oki-t4oki1
         dl1[i,1] = zero
         dl1[i,2] = (4.d0*g3m10-dkdr0)*(t4oki1/diff) $
           *(one+ahf*rl*dm2[i]/(forpi*r[i]^3*ahf*(rho[i]+rhoc)))
         dl2[i,1] = zero
         dl2[i,2] = ((4.d0-dkdt0)*(t4oki1/diff))/(cv0*tc) $
           *(one+ahf*rl*dm2[i]/(forpi*r[i]^3*ahf*(rho[i]+rhoc)))
;c
...
;c
         ...
         deds = dedt0*sorc0/(cv0*tc)
           ak1[i,1] = zero
           ak1[i,2] = zero
           ak1[i,3] = - glom1*4.d0/r[i]^2 - $
             glom1*(dl1[i,1]*drdm02 + dl1[i,2]*drdm[i,1] ) + dedr*drdm02
...
;c
         rlom = zero
         rlom1 = frft[i]*rlumx1
         horiz = rkl20*4.d0*ac3*t4oki1/rhoc^2
...
;c     $       rkl20*ahf*rlom/(rhoc*forpi*rz0^3)
      ...
      endif else if( i eq np1 ) then begin
;c
;c          outer boundary conditions
;c
         tau = gkap(n)*dm1(n)/(4.d0*forpi*r(np1)^2)
         tau = 1.5d0*tau/(one+1.5d0*tau)
         fac = (r(np1)-rphoto)/r(np1)
         ef1 = (one+ahf*fac)/(one+fac)
         ef3 = two/(one+fac)
         dl1(np1,1) = ef1*(4.d0*g3m1(n)-tau*dkdr(n))
;c     $      *(one-ahf*rl*dm2[i]/(forpi*r[i]^3*rho[i-1]))
         dl1(np1,2) = dl1(np1,1)
         dl1(np1,2) = ef1*(4.d0*g3m1(n)-tau*dkdr(n))
;c     $      *(one+ahf*rl*dm2[i]/(forpi*r[i]^3*rho[i-1]))
         dl2(np1,1) = ef1*(4.d0-tau*dkdt(n))/(t(n)*cv(n))
;c     $      *(one-ahf*rl*dm2[i]/(forpi*r[i]^3*rho[i-1]))
         dl2(np1,2) = zero
         glom = rlumgv/dm1(n) $
         ...
         ak1(np1,2) =-glom1*dl1(np1,1)*drdm(n,1)+glom*4.d0/r(n)^2 + $
...
;c
         rlumx = rlumgv
         horiz = rkl2(n)*4.d0*ac3*t(n)^4*v(n)^2/gkap(n)
...
;c
         ...
         g1p = g1(n)*p(n)-4.d0*ad3*t(n)^4*g3m1(n)
         g3m1ro = g3m1(n)/v(n)-4.d0*ad3*t(n)^3/cv(n)
         ag1(np1,1) = drdm(n,1)*g1p*(ahf*rl*v[i-1] - afac)
         ag1(np1,2) =-4.d0*gor(np1)/r(np1) + pi4g*rho(n) + $
...
;c
         ...
         ag1[i,2]=-4.d0*gor[i]/r[i]+pi4g*ahf*(rho[i-1]+rho[i])+ $
...
;c
         ...
         dl1[i,1] = ((-4.d0*g3m1[i-1]+dkdr[i-1])*(t4oki1/diff) $
         + (-dkdr[i-1]/wiowi1+4.d0*g3m1[i-1]*gkogk1/wowsq)/denom) $
           *(one-ahf*rl*dm2[i]/(forpi*r[i]^3*ahf*(rho[i]+rho[i-1])))
         dl1[i,2] = ((4.d0*g3m1[i]-dkdr[i])*(t4oki/diff) $
         + (dkdr[i]/wiowi1-4.d0*g3m1[i]*gkogk1/wowsq)/denom) $
           *(one+ahf*rl*dm2[i]/(forpi*r[i]^3*ahf*(rho[i]+rho[i-1])))
         dl2[i,1] = ((-4.d0+dkdt[i-1])*(t4oki1/diff) - $
         (dkdt[i-1]/wiowi1-4.d0*gkogk1/wowsq)/denom)/(cv[i-1]*t[i-1]) $
           *(one-ahf*rl*dm2[i]/(forpi*r[i]^3*ahf*(rho[i]+rho[i-1])))
         dl2[i,2] = ((4.d0-dkdt[i])*(t4oki/diff) + $
         (dkdt[i]/wiowi1-4.d0*gkogk1/wowsq)/denom)/(cv[i]*t[i]) $
           *(one+ahf*rl*dm2[i]/(forpi*r[i]^3*ahf*(rho[i]+rho[i-1])))
;c
;c          Keep track of the luminosity as a function of mass
;c
...
;c         if( i .eq. 3 ) ak1[i,1] = zero
         ak1[i,2] =-glom1*dl1[i,1]*drdm[i-1,1]+glom*4.d0/r[i-1]^2 + $
          glom*(dl1[i-1,1]*drdm(i-2,2)+dl1[i-1,2]*drdm[i-1,1] ) $
          +  dedr*drdm[i-1,1]
         ak1[i,3] = glom*dl1[i-1,2]*drdm[i-1,2] - glom1*4.d0/r[i]^2 - $
         ...
         endelse
         ak4[i,2] =-rkl2[i-1]*(glom1*dl1[i,1] -  dedr - glom*dl1[i-1,2] )
...
;c
         rlom = frft[i-1]*rlumx
         rlom1 = frft[i]*rlumx1
         horiz = rkl2[i-1]*4.d0*ac3*t4oki1*v[i-1]^2
...
;c
      if( iout ge 2 ) then begin
         print, ' AG1, AG2, AG3, AG4, AH4'
         for i=1,np1 do begin
            print, 'format(1x,i5,1p,10e12.4)', i,ag1[i,1],ag1[i,2],ag1[i,3],ag2[i,1], $
              ag2[i,2],ag3[i,1],ag3[i,2],ag4[i,1],ag4[i,2],ah4[i]
         endfor
         print, ' AH1, AH2, AH3, AP1, AP3, AP4'
         for i=1,np1 do begin
            print, 'format(1x,i5,1p,10e12.4)', i,ah1[i,1],ah1[i,2],ah2[i],ah3[i],ap1[i,1], $
              ap1[i,2],ap3[i,1],ap3[i,2],ap3[i,3],ap4[i]
         endfor
         print, ' The pulsation matrices, AK1, AK2, AK4'
         for i=1,np1 do begin
            print, 'format(1x,i5,1p,10e12.4)', i,ak1[i,1],ak1[i,2],ak1[i,3],ak1(i,4), $
