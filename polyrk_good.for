...
c
 ...
 1200 format(1x,27henter pulsation parameters:,/,
...
c 1000 format(1x,18hpolytrope with n =,f5.2,20H^h2}x^lh.5%0 ^Lxhx%=,f7.4)
 1000 format(1x,'Polytrope with n =',f5.2,' outer radius= ',f7.4)
      ...
      subroutine tau_dump( npts, xi, theta, rlam, fnpol )
      implicit real*8(a-h,o-z)
      character*80 filename
      dimension xi(npts), theta(npts)
c
      write(filename, 1000) nint(10.*fnpol)
      open (31, file=filename, status='unknown' )
      write(31,1105) npts
      do 5 i=1,npts
         write(31,1100) xi(i)/xi(npts), rlam*theta(i)**fnpol
   5  continue
      close(31)
      return
c
 1000 format('poly_',i2.2,'.txt')
 1105 format(i6)
 1100 format(1x,1p2g12.5)
      end
...
c
         ...
         call robeig(index,qomsq,fnpol,lval)
...
c
     ...
     $                      weight(i),stwait(i),i=1,n)
         ...
         if( fnpol .eq. 0. ) then
            k = iabs(index)
c            call pekeris( k, lval, r, wcross, n)
          endif
...
c
 1000 format(1h1,16h radial solution,/,8h  omsq =,1pe14.7,5X,
     ...
     $  2hdp,11x,6hweight/(1x,i3,1p,5e14.6))
...
c
 ...
 5503 format(1x,20h Norm. eigenvalue = ,1pe12.5,/,
